...
function Bookingscreen({match}) {
    ...
    useEffect(async() => {
        ...
        try {
            setloading(true);
            const data = await (await axios.post(process.env.REACT_APP_BASE_URL + "/api/rooms/getroombyid" , {roomid})).data;
            console.log(data);
            setroom(data);
            setloading(false);
    ...
    async function tokenHander(token) {
        ...
        try {
            setloading(true);
            const result = await axios.post(process.env.REACT_APP_BASE_URL + '/api/bookings/bookroom' , bookingDetails)
            setloading(false)
